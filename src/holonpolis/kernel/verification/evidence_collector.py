    ...
    def has_critical_issues(self) -> bool:
        ...
        for vr in self.verification_results:
            if not vr.passed and vr.errors:
                return True
        for to in self.tool_outputs:
            if to.exit_code != 0 and "error" in to.stderr.lower():
                return True
        return False
...
class EvidenceCollector:
    ...
    def __init__(self, task_id: str, iteration: int = 0):
    ...
    def set_summary(self, summary: str, acceptance: Optional[bool] = None) -> None:
